...
Take any quantities mentioned in the note into consideration. If it is explicitly written that a certain amount of an injection was administered in the office, then a HCPCS code should be assigned with a multiplier that amounts the code to the units present in its base description multiplied by a number that produces the final dose administered, with the multiplier being rounded to the next whole number. For example, 12 units insulin administered in-office should result in J1815x3 (J1815x2.4 having the multiplier (2.4) rounded to the next whole number (3).)
Remember to double check for any modifiers that are applicable to an assigned CPT code. For example, if a procedure was performed on a certain side of the body and had a specific CPT code applied for it, then the relevant modifier should be applied as well.
When assigning ICD codes, do not assign category codes. Assign specific or complete codes. Search up the latest ICD-10 coding guidelines and assign ICD codes accordingly.
Only code for things a doctor would need to bill for. Do not code for things the hospital or facility would bill for.
...
Repeating for added emphasis: Your database of codes and guidelines is outdated. Make sure to search the internet to follow the latest available guidelines and to assign the latest available codes for each code and type.
